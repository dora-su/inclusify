...
class TextPage extends React.Component {
  ...
  changeText() {
    ...
    let text_arr = (
      text.substring(0, index_c) +
      " ¶ " +
      text.substring(index_c)
    ).split(/(?=[ /n])|(?<=[ /n])/g);
    let new_text = [];
    ...
    for (let i = 0; i < text_arr.length; i++) {
      ...
      if (this.isBadWord(word)) {
        //if word is bad word from function
        ...
        new_text.push(
          <span>
            <ChangedWord
              index={i}
              original_word={word}
              synonyms={badwords[word]}
              replaceWord={this.replaceWord}
            />{" "}
          </span>
        ); //get synonyms from json file
      } else if (word == "¶") {
  ...
  copyToClipboard = (e) => {
    document.getElementsByTagName("textarea").length !== 0
      ...
      : document.getElementById("changed-text-area")[0].select();
    ...
    if (this.state.mode == 0) {
      ...
      text.push(
        <div
          tabindex="0"
          className="changed-c"
          id="changed-text-area"
          onKeyDown={this.onKeyDown}
        >
          <div
            onClick={this.toEdit}
            className="changed-text-c"
            onKeyDown={this.onKeyDown}
          ></div>
        ...
        <div>
          ...
          <Container className="page-container" onKeyDown={this.onKeyDown}>
            ...
            <div class="text-container" style={{ marginTop: "16px" }}>
              {text}
              <button onClick={this.toChanged} className="inclusify-btn btn-1">
                Submit
              </button>
              {document.queryCommandSupported("copy") &&
              document.getElementsByTagName("textarea") !== null && (
                <div className="copy-text">
                  <button className="inclusify-btn"onClick={this.copyToClipboard}>
                    {" "}
